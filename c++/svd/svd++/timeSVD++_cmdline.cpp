...
const char aggrOutFilePath[] = "/home/msushkov/Dropbox/Caltech/Caltech Classes/CS/CS 156b/submissions/aggregation/svd++/timeSVD++_150f_aggr.txt";
const char outputFilePath[] = "/home/msushkov/Dropbox/Caltech/Caltech Classes/CS/CS 156b/submissions/SVD++/timeSVD++_150f_123_86%of4_qual.txt";
...
double *meanRatingDates = new double[NUM_USERS];

double **alpha = new double*[NUM_USERS];
...
void initialize() {
    ...
    for (int i = 0; i < NUM_USERS; i++) {
        ...
        alpha[i] = new double[NUM_FEATURES];
...
double calculateTimeParam(int user, int timeArg) {
    return sign(timeArg - meanRatingDates[user]) * pow(abs(timeArg - meanRatingDates[user]), 0.4);
}

double predictRating(int movie, int user, int timeArg) {
	double result = GLOBAL_AVG_MOVIE_RATING + userBias[user] + movieBias[movie];

    double prod = calculateTimeParam(user, timeArg);
    
    // find the dot product
    for (int f = 0; f < NUM_FEATURES; f++)
        result += movieFeatures[movie][f] * (userFeatures[user][f] + alpha[user][f] * prod + norm[user] * sumY[user][f]);
...
double computeInSample(int numFeatures) {
    ...
    for (int i = 0; i < NUM_POINTS; i++) {
        ...
        double predicted = predictRating(currMovie - 1, currUser - 1, currTime);
...
double computeOutOfSample(int numFeatures) {
    ...
    while (getline(outOfSample, line)) {
        ...
        double predictedRating = predictRating(movie - 1, user - 1, actualTime);
...
void learn() {
    ...
    while (iterations < NUM_EPOCHS || (prevEout - Eout) > EPSILON) {
        ...
        for (int p = 0; p < NUM_POINTS; p++) {
            ...
            double error = rating - predictRating(movie - 1, user - 1, timeVal);
            //double error = rating - predictRating_training(movie - 1, userBiasVal, movieBiasVal, currSumY, norm[user - 1], userFeature, movieFeature);
            ...
            for (int k = 0; k < NUM_FEATURES; k++) {
                ...
                currAlpha[k] += (LRATE_ALPHA * (error - LAMBDA_alpha * currAlpha[k]));
                ...
                if (countForCurrUser == numMovies) { // user is going to change on the next data point
                    ...
                    for (int j = 0; j < numMovies; j++) {
                        double *currY = y[moviesRatedByCurrUser.at(j) - 1];
...
void getAvgUserRatings() {
    ...
    while (getline(dataFile, line)) {
        ...
        if (prevUser != currUser) {
            ...
            meanRatingDates[prevUser - 1] = totalDateForUser / index;
            totalDateForUser = currDate;
...
void outputAggr() {
    ...
    while (getline(aggrFile, line)) {
        ...
        double predictedScore = predictRating(movie - 1, user - 1, timeVal);
